...
export function SettingsView() {
  ...
  const persistUpdateBadge = useCallback(
    (next: boolean, latestVersion?: string) => {
  ...
  const handleWindowDecorationsChange = useCallback(
    (enabled: boolean) => {
      updateSettings({
        window: {
          ...(settings?.window ?? {}),
  ...
  const handleTrayVisibleChange = useCallback(
    (visible: boolean) => {
      updateSettings({
        window: {
          ...(settings?.window ?? {}),
  ...
  const handleGlobalQuickAddShortcutChange = (
    shortcut: GlobalQuickAddShortcutSetting,
  ) => {
    setGlobalQuickAddShortcut(shortcut);
    showSaved();
  };
  const handleUndoNotificationsChange = useCallback(
    (enabled: boolean) => {
